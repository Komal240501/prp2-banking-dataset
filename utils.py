...
def load_tables() -> dict[str, pd.DataFrame]:
    """
    Load all 10 tables from the bundled /data folder. Plain .csv is used if present;
    otherwise a same-named .csv.gz is used as a fallback (handy for the two large
    tables — transaction_list, card_transaction — to keep repo file sizes down).
    """
    tables = {}
    for name, filename in TABLE_FILES.items():
        ...
        if os.path.exists(path):
            ...
            raise FileNotFoundError(f"Missing {filename} (or {filename}.gz) in /data.")
